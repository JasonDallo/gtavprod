    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {0, 255, 0}, "^0E pour utiliser l'ATM");
        end
        atBank = true
        if IsControlJustPressed(1, 38)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Vous devez sortir du véhicule!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Cette personne n'est pas sur l'ile!");
    end
  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Cette personne n'est pas assez proche!");
  end
end)
