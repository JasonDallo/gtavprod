...
function tick()
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Livrez jusqu'a la ~g~destination~w~.", 10000)
            text1 = true
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "Vous recevez $"..currentMission[4], 5000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "Arrivé. Détachez la ~o~remorque~w~ avec ~r~H~w~", 15000)
        end
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "Devenir chauffeur poid-lourd ? Appuye sur ~r~N+~w~.", 500)
...
function GUI.init()
    GUI.loaded = true
    GUI.addTitle("Vous etes devenu chauffeur.", 0.425, 0.19, 0.45, 0.07 )
    GUI.addDesc("Choisissez votre cabine.", 0.575, 0.375, 0.15, 0.30 )
    ...
    GUI.addButton(0, "Fermer Menu", GUI.exit, 0.35, 0.50, 0.3, 0.05 )
    ...
    GUI.addButton(1, "Fermer Menu", GUI.exit, 0.35, 0.55, 0.3, 0.05)
end
